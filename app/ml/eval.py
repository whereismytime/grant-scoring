...
def main():
    ...
    # «истинную» метку берём как то, что генерил train.py (правила)
    # если есть реальный датасет — подставь его approved.
    y_true = []
    y_rule = []
    y_ml = []
    p_ml = []
    for r in df.itertuples(index=False):
        a = Applicant(distance_km=float(r.distance_km),
        ...
        p = ml.prob(x); p_ml.append(p); y_ml.append(int(p>=0.5))
        y_true.append(y_r)  # если есть реальная разметка — подставь её

    y_true = np.array(y_true); y_rule = np.array(y_rule)
    y_ml = np.array(y_ml); p_ml = np.array(p_ml)

    print("AUC(ML vs y_true):", round(roc_auc_score(y_true, p_ml), 3))
    print("ACC rule:", round(accuracy_score(y_true, y_rule), 3),
          "ACC ML:", round(accuracy_score(y_true, y_ml), 3))
    diff = (y_rule != y_ml)
    print("Доля расхождений ML vs rules:", round(diff.mean(), 3))
    print("Confusion (ML vs rules):\n", confusion_matrix(y_rule, y_ml))

    # покажем 5 кейсов, где ML ≠ правило
    show = df.loc[diff].copy()
    show["rule"] = y_rule[diff]; show["ml"] = y_ml[diff]; show["p_ml"] = np.round(p_ml[diff],3)
